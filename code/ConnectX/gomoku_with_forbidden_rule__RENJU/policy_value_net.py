...
class PolicyValueNet():
    def __init__(self, model_file=None):
        ...
        # 1. Input:
        # self.input_states = tf.placeholder(tf.float32, shape=[None, 3, 15, 15])
        self.input_states = tf.placeholder(tf.float32, shape=[None, 3, 8, 8])
        ...
        self.action_conv = tf.layers.conv2d(inputs=self.conv3, filters=3,
                                            ...
                                            activation=tf.nn.relu)
        # Flatten the tensor
        # self.action_conv_flat = tf.reshape(self.action_conv, [-1, 3 * 15 * 15])
        self.action_conv_flat = tf.reshape(self.action_conv, [-1, 3 * 8 * 8])
        ...
        self.evaluation_conv = tf.layers.conv2d(inputs=self.conv3, filters=2,
                                                ...
                                                data_format="channels_last",
                                                activation=tf.nn.relu)
        # self.evaluation_conv_flat = tf.reshape(self.evaluation_conv, [-1, 2 * 15 * 15])
        self.evaluation_conv_flat = tf.reshape(self.evaluation_conv, [-1, 2 * 8 * 8])
        ...
        # self.mcts_probs = tf.placeholder(tf.float32, shape=[None, 15 * 15])
    ...
    def policy_value_fn(self, board):
        ...
        # current_state = np.ascontiguousarray(board.current_state().reshape(-1, 3, 15, 15))
        current_state = np.ascontiguousarray(board.current_state().reshape(-1, 3, 8, 8))
...
class PolicyValueResidualNet():
    def __init__(self, model_file=None):
        self.model_file = model_file
        
        # policy weight and value weight
        self.loss_weight = [1.0,0.1] 
