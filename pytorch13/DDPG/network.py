...
class Critic(nn.Module):
    ...
    def forward(self, state, action):
        # we'd better do the batch norm first before the relu
        s = F.relu(self.bn1(self.fc1(state)))
        s = self.bn2(self.fc2(s))
        # according to the original paper, we include the actions here
        a = F.relu(self.separate_action_layer(action))
        # combine the state_value and action_value, then pass the final layer
        q_value = self.q(F.relu(T.add(s, a)))
...
class Actor(nn.Module):
    ...
    def forward(self, state):
        ...
        # according to the original paper, we use the "tanh" at 
        # the final layer to bound the actions
        return T.tanh(self.action_optimal(action))
