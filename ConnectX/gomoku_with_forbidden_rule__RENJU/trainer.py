...
class Trainer(object):
    ...
    def get_equi_data(play_data):
        ...
        for state, mcts_porb, winner in play_data:
            for i in [1, 2, 3, 4]:
                ...
                # equi_mcts_prob = np.rot90(np.flipud(mcts_porb.reshape(15,15)), i)
                equi_mcts_prob = np.rot90(np.flipud(mcts_porb.reshape(8,8)), i)
    ...
    def policy_update(self,game_data):
        ...
        for i in tqdm(range(5), ascii=True, desc='Running Policy Update'): 
    ...
    def feed(self, data):
        self.counter +=1
        print('We have already fed data %d times' %self.counter)
