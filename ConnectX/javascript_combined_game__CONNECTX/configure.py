...
def run_configure():
    ...
    while True:
        input1 = input("Config 1: Please input the size of board. (size >= 3)\n"
                       "size ({}) = ".format(size))
        try:
            input_int = size if len(input1) == 0 else int(input1)
            if input_int < 3:
                print("size should be greater than or equal to 3. Please try again.\n")
                continue
            size = input_int
        except:
    ...
    while True:
        input2 = input("Config 2: Please input how many pieces in a row. (n >= 3 and n <= size)\n"
                       "n ({}) = ".format(n_in_a_row))
        try:
            input_int2 = n_in_a_row if len(input2) == 0 else int(input2)
            if input_int2 < 3:
                print("n should be greater than or equal to 3. Please try again.\n")
                continue
            if input_int2 > size:
                print("n should be less than or equal to size. Please try again.\n")
                continue
