...
class TrainPipeline():
    def __init__(self, init_model=None, board_width=6, board_height=6,
                 n_in_row=4, n_playout=400, use_gpu=False, is_shown=True,
    ...
    def collect_selfplay_data(self, n_games=1):
        ...
        for i in tqdm(range(n_games), ascii=True, desc='Collect Data'):
    ...
    def policy_update(self):
        ...
        for i in tqdm(range(self.epochs), ascii=True, desc='Policy Update'):
    ...
    def run(self):
        ...
        with open("info/"+str(self.board)+"_loss_"+self.output_file_name+".txt",'w') as loss_file:
            loss_file.write("self-play number, loss, entropy\n")
        with open("info/"+str(self.board)+"_win_ration"+self.output_file_name+".txt", 'w') as win_ratio_file:
            win_ratio_file.write("self-play number , pure_MCTS rollouts number, winning rate\n")

        for i in tqdm(range(self.game_batch_num), ascii=True, desc='Running Train'):
