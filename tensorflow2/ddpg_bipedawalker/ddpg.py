# DDPG is basically a policy gradient method enforced using a DNN
# but for DDPG, we can also see it as an approximating Q-Learnig algorithm
# see CS285 "Deep RL with Q-fincton" for more Information.


# read the paper while writing down the things we need to implement.

# Need a replay buffer
# Need class for a target Q network (functon of s, a)
# Use Batch Norm
# Handle exploration (the policy is deterministic)
# We have two actor and two critic networks, a target for each
# Updates are soft
# the target actor is just the evaluation actor plus noise process
